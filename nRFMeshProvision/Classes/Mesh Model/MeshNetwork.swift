...
extension MeshNetwork {
    ...
    ///
    /// - parameter elements:    The number of node's elements. Each element will be
    ...
    func allocateNextAvailableUnicastAddress(for elements: UInt16, elementsUsing provisioner: Provisioner) -> Address? {
        ...
        for range in provisioner.allocatedUnicastRange {
            ...
            for _ in currentIndex..<sortedNodes.count {
                ...
                if address > node.unicastAddress + node.elementsCount {
                    continue
                }
                // If we found a space before the current node, return the address.
                if address + elements < node.unicastAddress {
                    return address
                }
                // Else, mode the address to the end of the node's assigned addersses.
                address = node.unicastAddress + node.elementsCount
                
                // If the new address is outside of the range, go to the next one.
                if address + elements > range.highAddress {
                    break
                }
            }
