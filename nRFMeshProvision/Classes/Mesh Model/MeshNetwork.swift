...
extension MeshNetwork {
    ...
    ///
    /// - parameter count:       The number of node's elements. Each element will be
    ...
    func allocateNextAvailableUnicastAddress(for count: UInt16, elementsUsing provisioner: Provisioner) -> Address? {
        ...
        for range in provisioner.allocatedUnicastRange {
            ...
            for _ in currentIndex..<sortedNodes.count {
                ...
                if address + count < node.unicastAddress {
                    return address
                }
                ...
                if address + count > range.highAddress {
                    break
                }
            }
