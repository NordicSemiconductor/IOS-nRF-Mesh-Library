//
//  GenericOnOffUnacknowledgedSetControllerState.swift
//  nRFMeshProvision
//
...
class GenericOnOffSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: GenericOnOffSetUnacknowledgedMessage
        if let targetState = targetState {
            if let targetTransitionParams = targetStateTransitionParameters {
                message = GenericOnOffSetUnacknowledgedMessage(withTargetState: targetState,
