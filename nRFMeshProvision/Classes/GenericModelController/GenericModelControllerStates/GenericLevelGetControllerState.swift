//
//  GenericLevelGetControllerState.swift
//  nRFMeshProvision
//
...
class GenericLevelGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        let message = GenericLevelGetMessage()
        //Send to destination
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is GenericLevelStatusMessage {
                    let genericOnOffStatus = result as! GenericLevelStatusMessage
                    target.delegate?.receivedGenericLevelStatusMessage(genericOnOffStatus)
