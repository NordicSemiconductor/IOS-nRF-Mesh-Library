//
//  SceneStoreUnacknowledgedControllerState.swift
//  nRFMeshProvision
//
...
class SceneStoreUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: SceneStoreUnacknowledgedMessage
        if let sceneNumber = sceneNumber {
            message = SceneStoreUnacknowledgedMessage(withSceneNumber: sceneNumber)
        } else {
        ...
        target.delegate?.sentSceneStoreUnacknowledged(destinationAddress);
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneRegisterStatusMessage {
                    let sceneStoreUnacknowledgedStatus = result as! SceneRegisterStatusMessage
                    target.delegate?.receivedSceneRegisterStatusMessage(sceneStoreUnacknowledgedStatus)
                    let nextState = SleepConfiguratorState(withTargetProxyNode: target, destinationAddress: destinationAddress, andStateManager: stateManager)
                    target.switchToState(nextState)
                }
            } else {
