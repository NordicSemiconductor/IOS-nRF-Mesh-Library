//
//  SceneRecallControllerState.swift
//  nRFMeshProvision
//
...
class SceneRecallControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: SceneRecallMessage
        if let sceneNumber = sceneNumber {
            message = SceneRecallMessage(withSceneNumber: sceneNumber)
        } else {
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneStatusMessage {
                    let sceneRecallStatus = result as! SceneStatusMessage
                    target.delegate?.receivedSceneStatusMessage(sceneRecallStatus)
