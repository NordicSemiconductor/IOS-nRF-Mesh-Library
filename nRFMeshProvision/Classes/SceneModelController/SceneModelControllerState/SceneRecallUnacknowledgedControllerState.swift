//
//  SceneRecallUnacknowledgedControllerState.swift
//  nRFMeshProvision
//
...
class SceneRecallUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: SceneRecallUnacknowledgedMessage
        if let sceneNumber = sceneNumber {
            message = SceneRecallUnacknowledgedMessage(withSceneNumber: sceneNumber)
        } else {
        ...
        target.delegate?.sentSceneRecallUnacknowledged(destinationAddress);
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneStatusMessage {
                    let sceneRecallUnacknowledgedStatus = result as! SceneStatusMessage
                    target.delegate?.receivedSceneStatusMessage(sceneRecallUnacknowledgedStatus)
                    let nextState = SleepConfiguratorState(withTargetProxyNode: target, destinationAddress: destinationAddress, andStateManager: stateManager)
                    target.switchToState(nextState)
                }
            } else {
