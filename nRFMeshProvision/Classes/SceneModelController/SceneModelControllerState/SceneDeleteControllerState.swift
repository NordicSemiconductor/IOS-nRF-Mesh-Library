//
//  SceneDeleteControllerState.swift
//  nRFMeshProvision
//
...
class SceneDeleteControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: SceneDeleteMessage
        if let sceneNumber = sceneNumber {
            message = SceneDeleteMessage(withSceneNumber: sceneNumber)
        } else {
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneRegisterStatusMessage {
                    let sceneDeleteStatus = result as! SceneRegisterStatusMessage
                    target.delegate?.receivedSceneRegisterStatusMessage(sceneDeleteStatus)
