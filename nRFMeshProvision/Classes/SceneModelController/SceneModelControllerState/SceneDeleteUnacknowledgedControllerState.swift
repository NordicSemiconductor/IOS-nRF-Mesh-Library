//
//  SceneDeleteUnacknowledgedControllerState.swift
//  nRFMeshProvision
//
...
class SceneDeleteUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    public func setSceneNumber(withSceneNumber aSceneNumber: Data) {
        sceneNumber = aSceneNumber
    }
    ...
    func execute() {
        var message: SceneDeleteUnacknowledgedMessage
        if let sceneNumber = sceneNumber {
            message = SceneDeleteUnacknowledgedMessage(withSceneNumber: sceneNumber)
        } else {
        ...
        target.delegate?.sentSceneDeleteUnacknowledged(destinationAddress);
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneRegisterStatusMessage {
                    let sceneDeleteUnacknowledgedStatus = result as! SceneRegisterStatusMessage
                    target.delegate?.receivedSceneRegisterStatusMessage(sceneDeleteUnacknowledgedStatus)
                    let nextState = SleepConfiguratorState(withTargetProxyNode: target, destinationAddress: destinationAddress, andStateManager: stateManager)
                    target.switchToState(nextState)
                }
            } else {
                print("ignoring non SceneRegisterStatus message")
            }
        }
    }
