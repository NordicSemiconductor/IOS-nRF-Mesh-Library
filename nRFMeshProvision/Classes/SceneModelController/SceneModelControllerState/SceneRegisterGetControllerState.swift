//
//  SceneRegisterGetControllerState.swift
//  nRFMeshProvision
//
...
class SceneRegisterGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        let message = SceneRegisterGetMessage()
        //Send to destination
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneRegisterStatusMessage {
                    let sceneRegisterStatus = result as! SceneRegisterStatusMessage
                    target.delegate?.receivedSceneRegisterStatusMessage(sceneRegisterStatus)
                ...
                print("ignoring non SceneRegisterGet message")
            }
        }
    }
