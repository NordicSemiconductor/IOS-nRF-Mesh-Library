...
class SceneGetControllerState: NSObject, GenericModelControllerStateProtocol {
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SceneStatusMessage {
                    let sceneGet = result as! SceneStatusMessage
                    target.delegate?.receivedSceneStatusMessage(sceneGet)
                ...
                print("ignoring non SceneGet message")
            }
        }
    }
