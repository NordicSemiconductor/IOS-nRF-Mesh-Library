//
//  ModelSubscriptionAddConfiguratorState.swift
//  nRFMeshProvision
//
...
class ModelSubscriptionAddConfiguratorState: NSObject, ConfiguratorStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Model Subscription Add"
    }
    
    func execute() {
        let message = ModelSubsriptionAddMessage(withElementAddress: elementAddress,
        ...
        for aPayload in payloads! {
            ...
            if data.count <= target.basePeripheral().maximumWriteValueLength(for: .withoutResponse) {
                print("Sending model subscription address add data: \(data.hexString())")
                ...
                for aSegment in segmentedProvisioningData {
                    print("sending model subscription address add segment: \(aSegment.hexString())")
