...
class LightCtlSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Ctl Unacknowled Set"
    }
    ...
    func execute() {
        var message: LightCtlSetUnacknowledgedMessage
        if let targetState = targetState {
            if let targetTransitionParams = targetStateTransitionParameters {
                message = LightCtlSetUnacknowledgedMessage(withTargetState: targetState,
        ...
        target.delegate?.sentLightCtlSetUnacknowledged(destinationAddress);
