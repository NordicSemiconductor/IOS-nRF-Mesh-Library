...
class LightHslSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Hsl Unacknowled Set"
    }
    ...
    func execute() {
        var message: LightHslSetUnacknowledgedMessage
        if let targetState = targetState {
            if let targetTransitionParams = targetStateTransitionParameters {
                message = LightHslSetUnacknowledgedMessage(withTargetState: targetState,
        ...
        target.delegate?.sentLightHslSetUnacknowledged(destinationAddress);
