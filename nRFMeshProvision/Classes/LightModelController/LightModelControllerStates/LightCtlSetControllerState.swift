//
//  LightLightnessSetControllerState.swift
//  nRFMeshProvision
//
...
class LightCtlSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is LightCtlStatusMessage {
                    let lightCtlStatus = result as! LightCtlStatusMessage
                    target.delegate?.receivedLightCtlStatusMessage(lightCtlStatus)
                ...
                print("ignoring non LightCtlMessage")
            }
        }
    }
