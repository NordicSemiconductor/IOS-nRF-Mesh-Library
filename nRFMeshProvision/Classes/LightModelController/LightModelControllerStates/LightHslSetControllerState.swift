//
//  LightHslSetControllerState.swift
//  nRFMeshProvision
//
...
class LightHslSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is LightHslStatusMessage {
                    let lightHslStatus = result as! LightHslStatusMessage
                    target.delegate?.receivedLightHslStatusMessage(lightHslStatus)
                ...
                print("ignoring non LightHslMessage")
            }
        }
    }
