...
class LightLightnessSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: LightLightnessSetMessage
        if let targetState = targetState {
            if let targetTransitionParams = targetStateTransitionParameters {
                message = LightLightnessSetMessage(withTargetState: targetState,
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is LightLightnessStatusMessage {
                    let lightLightnessStatus = result as! LightLightnessStatusMessage
                    target.delegate?.receivedLightLightnessStatusMessage(lightLightnessStatus)
