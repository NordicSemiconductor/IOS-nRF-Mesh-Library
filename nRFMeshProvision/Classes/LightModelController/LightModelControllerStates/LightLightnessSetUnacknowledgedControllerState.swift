...
class LightLightnessSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Lightness Unacknowled Set"
    }
    ...
    func execute() {
        var message: LightLightnessSetUnacknowledgedMessage
        if let targetState = targetState {
            if let targetTransitionParams = targetStateTransitionParameters {
                message = LightLightnessSetUnacknowledgedMessage(withTargetState: targetState,
        ...
        target.delegate?.sentLightLightnessSetUnacknowledged(destinationAddress);
