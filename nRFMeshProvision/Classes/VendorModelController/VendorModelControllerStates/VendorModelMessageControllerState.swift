...
class VendorModelMessageControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Vendor Model Message"
    }
    
    public func setOpcode(aOpcode: Data) {
        opcode = aOpcode
    }
    
    public func setParams(aParams: Data) {
        params = aParams
    }
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is VendorModelStatusMessage {
                    let vendorModelStatus = result as! VendorModelStatusMessage
